...
// src/parse.ts
var JsonParseStream = class extends import_node_stream.Transform {
...
var JsonStringifyStream = class extends import_node_stream2.Transform {
  _open;
  _sep;
  _close;
  _first = true;
...
async function stringifyArray(values, open = "[\n", sep = "\n,\n", close = "\n]\n") {
  return new Promise((resolve, reject) => {
    ...
    stream.on("error", reject);
    import_node_stream3.Readable.from(values).pipe(stream);
  });
}
